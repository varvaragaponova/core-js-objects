...
function makeWord(lettersObject) {
  const result = new Array(Object.values(lettersObject).flat(2).length);
...
function sellTickets(queue) {
  ...
  if (counter >= queue[queue.length - 1] || queue.length === 0) {
    result = true;
  }
...
function group(array, keySelector, valueSelector) {
  ...
  const result = arrKeys.reduce((acc, item) => {
    if (acc.has(item)) return acc;
    const cities = array
      .filter((elem) => {
        return Object.values(elem).includes(item);
      })
      .map(valueSelector);
    acc.set(item, cities);
    return acc;
